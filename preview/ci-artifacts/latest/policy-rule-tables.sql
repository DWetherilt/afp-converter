...
insert into policy_rule_catalog(rule_id, realm, category, rule_text, source_ref, mutable_by, enabled, updated_at) values
  ...
  ('PM-ID-002', 'pm', 'identifier-governance', 'PM refresh pipeline must publish pm/reports/identifier-registry.json for auditability of persisted identifiers.', 'build.gradle:identifierRegistryReport', 'human', 1, current_timestamp),
...
insert into pm_data_dictionary(object_name, object_type, realm, definition, source_ref, naming_pattern, updated_at) values
  ...
  ('realm_knowledge_link_gate_report', 'report', 'pm', 'Knowledge link gate status report showing unresolved per-realm decision references with warn/fail semantics.', 'tools/realm_knowledge_link_gate.py', 'snake_case', current_timestamp),
  ...
  ('release_name', 'presentation_field', 'pm', 'Human-friendly release label emitted in pm/reports/version.json.', 'build.gradle:versionStamp', 'title_case', current_timestamp),
  ...
  ('identifier_registry_report', 'report', 'pm', 'Audit export of all persisted identifiers generated to pm/reports/identifier-registry.json.', 'build.gradle:identifierRegistryReport', 'snake_case', current_timestamp),
