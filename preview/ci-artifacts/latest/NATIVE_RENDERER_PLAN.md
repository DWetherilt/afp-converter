...
1. extend Workstream F cross-exam to operand-level tracing (PTX control-sequence payload bytes vs PDF text-state/text-show operators and operands), then back-port at least one proven mapping into `AfpNativePdfRenderer` PTX handling.
