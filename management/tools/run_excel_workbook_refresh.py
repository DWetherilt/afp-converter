...
def main() -> int:
    ...
    parser.add_argument("--xlsx", required=True, help="Path to management/docs/project-plan-progress.xlsx")
    parser.add_argument("--json", required=True, help="Path to management/pm/reports/project-plan-progress-data.json")
    parser.add_argument(
        "--script",
        default="management/tools/run_excel_workbook_refresh.applescript",
...
def write_status(mode: str, excel_ok: bool, excel_error: str) -> None:
    ...
    out = Path("management/pm/reports/workbook-refresh-status.json")
