...
upsert_pm_text() {
  ...
  /usr/bin/sqlite3 management/pm/state/pm-realm.sqlite \
...
checkpoint_reports() {
  ...
  local cp_dir="management/pm/checkpoints/recovery-doctor-local/${ts}"
  mkdir -p "$cp_dir"
  for f in \
...
prune_checkpoints() {
  local base_dir="management/pm/checkpoints/recovery-doctor-local"
...
if [[ "$RECOVERY_STRICT_CONDITIONAL_BINARIES" == "true" || "$RECOVERY_STRICT_CONDITIONAL_BINARIES" == "1" ]]; then
  STRICT_MODE=true REQUIRE_CONDITIONAL=true management/tools/restore_binary_artifacts.sh
else
...
upsert_pm_text management/pm/reports/project-plan-progress-data.json
upsert_pm_text management/pm/reports/issues-log-tickle.json
upsert_pm_text management/pm/reports/issues-effectiveness.json

python3 management/tools/sql_drift_summary.py --output management/pm/reports/sql-drift-summary.json --enforce
upsert_pm_text management/pm/reports/sql-drift-summary.json
