...
PID_FILE="$ROOT_DIR/management/pm/state/pmconsole-live.pid"
LOG_FILE="$ROOT_DIR/management/pm/state/pmconsole-live.log"
INTERVAL_SECONDS="${PMCONSOLE_LIVE_INTERVAL_SECONDS:-5}"
TOP_DECISIONS="${PMCONSOLE_LIVE_TOP:-7}"
PMCONSOLE_BIN="$ROOT_DIR/management/pm-console/build/install/pmconsole/bin/pmconsole"

cmd="${1:-status}"

mkdir -p "$ROOT_DIR/management/pm/state"
