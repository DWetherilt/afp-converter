...
- `management/pm/state` is used for PM databases (not application folders).
- `management/pm/reports` is used for PM report outputs.
- `management/pm/checkpoints` is used for rollback artifacts.
- `management/pm/workflow/workflow-manifest.json` exists and is wired.
- `management/pm/policy/policy-rule-tables.sql` exists and is consumed.

## C. Runtime behavior parity
- PM console command surface includes `status`, `tools`, and `refresh`.
- Workflow execution is adapter-based via `management/tools/pm_workflow.py`.
